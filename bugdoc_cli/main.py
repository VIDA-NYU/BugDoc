...
def main(args):
    ...
    for _ in range(workers):
        os.system(
            "bugdoc-cli-worker --file %s --receive %s --send %s & disown"
    ...
    input_dict = {p["name"]: p["values"] for p in configuration["parameters"]}

    logging.info("Debugging: " + configuration["entry_point"])

    logging.debug("Parameters: " + str(input_dict.keys()))
    autodebug = StackedShortcut(
        separator="#", origin="debug", max_iter=budget, send=send, receive=receive
    )
    ...
    autodebug = DebuggingDecisionTrees(
        separator="#", max_iter=budget, num_tests=10, origin="debug", send=send, receive=receive
    )
...
def run():
    ...
    parser.add_argument(
        "--receive", type=int, help="Socket port used to receive messages from worker"
    )
    parser.add_argument(
        "--workers", type=int, help="number of parallel workers to execute pipelines"
    )
