import sys, ast
import json
from bugdoc.algos.stacked_shortcut import AutoDebug
...
def execute(experiements_path,max_iter = sys.maxsize, k=4,use_score=False):
    ...
    for experiment in alllines:
        ...
        believedecisive, num_tests, new_tests = autodebug.run(experiment_name+".vt",param_space,['result'])
        result_dict = {'believedecisive': believedecisive, 'total':num_tests, 'created': new_tests}
        f = open(experiment_name+"_stacked_%d.res"%(max_iter),"a")
