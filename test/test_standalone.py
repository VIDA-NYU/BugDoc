...
class TestStackedShortcutStandalone(unittest.TestCase):
    ...
    def test_algorithm_execution(self):
        ...
        parameter_space = {"param1": [0.1, 0.6], "param2": [0.2, 0.7]}

        # This should run without errors
        try:
            algo.run("entry_point", parameter_space, outputs=["result"])
    ...
    def test_run_with_historical_runs_skips_initial_generation(self):
        ...
        history = ([[0.6, 0.7, True]], [[0.6, 0.7, True]])
        algo = StackedShortcutStandalone(function=self.mock_pipeline, max_iter=10)
        parameter_space = {"param1": [0.6], "param2": [0.7]}

        result = algo.run(
            "entry_point", parameter_space, outputs=["result"], historical_runs=history
        )
